...
const SystemRole: React.FC<SystemRoleProps> = ({ setMessages }) => {
  function handleChange(value: SegmentedValue) {
    const selectedOption = systemRoles.find((option) => option.value === value);
    if (!selectedOption) return;
    setMessages([{ role: "system", content: selectedOption.content }]);
  }
  return <Segmented block options={systemRoles} onChange={handleChange} />;
};
